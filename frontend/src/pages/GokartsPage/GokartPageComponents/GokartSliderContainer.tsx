      ...
      {gokarts && gokarts.length > 0 ? (
        ...
        >
          {gokarts.map((gokart, i) => (
            <SwiperSlide key={i} className="lg:w-[450px] md:w-[400px] sm:w-[350px] w-[300px]">
              <img
                src={imagesPath + gokart.image}
                alt={`Slide ${i + 1}`}
                className="aspect-square"
              />
              <h3 className="xl:text-4xl lg:text-3xl md:text-2xl text-md xl:my-4 md:my-3 my-2 text-center text-main-default">
              {gokart.name}
              </h3>
              <Divider className="w-10/12 mx-auto h-[0.4dvh] rounded-lg" />
              <p className="lg:text-lg md:text-md text-center text-sm">{gokart.description}</p>
            </SwiperSlide>
